        ...
        for x in range(width):
            for y in range(height):
                ...
                for a in range(len(actions)):
                    ...
                    for k,ns in enumerate(next_state):
                        prob = p if k==a else q
                        if 0<=ns[0]<world.shape[0] and 0<=ns[1]<world.shape[1]:
                            eu+= prob*utilities[ns]
                        else:
                            eu+= prob*utilities[x,y]
                    exp_utilities.append(eu)
                utilities[x,y] = world[x,y]['reward']+gamma*max(exp_utilities)
                delta = max(delta,np.abs(utilities[x,y]-u))
        if delta < epsilon*(1-gamma)/gamma:
            break
    optimal_policy = np.zeros(world.shape,dtype = int)
    for x in range(width):
            for y in range(height):
                if world[x,y]['isterminal'] or world[x,y]['iswall']:
                    continue
                ...
                optimal_policy[x,y] = np.argmax(exp_utilities)
    for x in range(width):
            for y in range(height):
                policy[x,y] = actions[optimal_policy[x,y]]
